...
public class AuthOutboxProcessor {
    private static final Logger _log = LoggerFactory.getLogger(AuthOutboxProcessor.class);
    ...
    public void processMessage(UUID messageId) {
        ...
        AuthOutBox entity = this._repo.findById(messageId)
                .orElseThrow(() -> {
    ...
    public void handleFailure(UUID messageId, Throwable ex) {
        ...
        AuthOutBox entity = this._repo.findById(messageId).orElse(null);
        ...
        int maxRetries = this._authProperties.getOutbox().getMaxRetries();
    ...
    private String resolveEventId(AuthOutBox entity) {
