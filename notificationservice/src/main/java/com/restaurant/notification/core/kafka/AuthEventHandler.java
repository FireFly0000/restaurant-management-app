...
public class AuthEventHandler {
    private static final Logger _log = LoggerFactory.getLogger(AuthEventHandler.class);
    ...
    @KafkaListener(
            topics = "auth.forgot.password.request",
            containerFactory = "kafkaListenerContainerFactory",
            concurrency = "${app.notification.kafka.consumer.concurency}"
    )
    public void handleResetPasswordRequest(
            List<ConsumerRecord<String, SendEmailEvent>> records,
            Acknowledgment ack
    ){
        sendEmail(records);
    ...
    private void sendEmail(
        ...
        _log.info("sendEmail, Prepare to send {} email", records.size());
        try {
